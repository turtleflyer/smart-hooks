...
const checkTypes: TestDescription = (p) => [
  ...
  () => {
    ...
    const id1 = 1;
    const initValue1 = () => (sn: string | number) => (sn === '' ? false : 'go');
    let testType1: (n: number, k: { a: boolean }) => boolean | string | number;
    const bindValue1 = () => (snb: string | number | boolean) => (snb === 0 ? false : 'go');
    ...
    const bindValue2_1: InterstateParam<string | boolean> = (s: number | string | boolean) =>
      'left';
    const CheckTypesExplicitly2 = () => {
      const [useSubscribe, setInterstate] = useInterstate<string | boolean>(id2, initValue2);
    ...
    const initValue3: InterstateInitializeParam<(p: string) => boolean | string> = () => (
      sn: string | number
    ) => sn === '';
    let testType3: (n: string, k: { a: boolean }) => boolean | string | number | { b: symbol };
    const bindValue3: InterstateParam<(p: string) => boolean | string> = () => (snb: string) =>
      snb === '';
